...
struct CalendarEvent: Identifiable, Sendable, Hashable {
    ...
    /// can read — a cancelled meeting can still arrive as `.confirmed`. Both
    /// spellings, because the wire uses the American one and the British one
    /// turns up from other clients.
