...
final class RichClipboardTests: XCTestCase {
    ...
    func testListHTMLUsesTheSameSpareTags() {
        let html = RichClipboard.listHTML(["Ship the PDF", "Ask Jordan <later>"])
        XCTAssertEqual(html, "<ul><li>Ship the PDF</li><li>Ask Jordan &lt;later&gt;</li></ul>")
        XCTAssertFalse(html.contains("<ol"))
        XCTAssertEqual(RichClipboard.listHTML(["", "  "]), "")
    }
}
